...
import { requestPasswordResetSchema, validateBody } from "@/lib/validations"
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validation = validateBody(requestPasswordResetSchema, body)
    if (!validation.success) return validation.error

    const email = canonicalEmail(validation.data.email)
    const emailLimit = checkRateLimitByKey(`otp:password-reset:request:${request.nextUrl.pathname}`, email, {
      limit: 3,
      windowSeconds: 900,
    })
      ...
      return NextResponse.json(
        {
          ...
          resendCooldownSeconds: secondsUntil(pendingVerification.resendAvailableAt),
        },
        { status: 200 }
      )
    }
