...
function SocialApprovals() {
  ...
  if (!allowed) {
    return (
      <div className="flex h-[60vh] flex-col items-center justify-center gap-2 text-center">
        ...
        <div className="text-lg font-bold">Social Approvals khusus One Above All</div>
        <p className="max-w-sm text-sm text-muted-foreground">Halaman ini cuma buat role tertinggi.</p>
      </div>
    );
  }
  ...
  return (
    <div>
      <PageHeader title="Social Approvals" subtitle="Review & approve post yang di-generate otomatis sebelum tayang lewat Buffer." icon={<Megaphone className="h-6 w-6 text-primary" />} />
      <div className="p-4 md:p-8 space-y-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-muted-foreground">{items.length} draft nunggu approval</span>
        </div>

        {data && !data.configured && (
          <div className="rounded-2xl border border-dashed border-amber-300 bg-amber-50 p-6 text-center">
            <div className="font-bold text-amber-800">Buffer belum tersambung</div>
            <p className="mt-1 text-sm text-amber-700">Token Buffer (`BUFFER_API_TOKEN`) belum diset di server. Kasih API key Buffer ke admin buat ngaktifin.</p>
          </div>
        )}
        {data?.error && (
          <div className={cn("rounded-xl border px-4 py-2 text-sm", data.rateLimited ? "border-amber-200 bg-amber-50 text-amber-800" : "border-rose-200 bg-rose-50 text-rose-700")}>
            {data.error}
            {data.stale && <span className="ml-1 opacity-70">(data terakhir, {items.length} draft)</span>}
          </div>
        )}
        {drafts.isError && !data && (
          <div className="rounded-xl border border-rose-200 bg-rose-50 px-4 py-2 text-sm text-rose-700">Gagal narik draft dari server. Coba Refresh.</div>
        )}
        ...
        {data?.configured && !drafts.isLoading && items.length === 0 && !data.error && (
          <div className="rounded-2xl border border-dashed border-border bg-card p-10 text-center text-sm text-muted-foreground shadow-sm">
            <Check className="mx-auto mb-2 h-8 w-8 text-emerald-400" /> Semua beres — gak ada draft yang nunggu approval. 🎉
          </div>
        )}
  ...
  if (err) {
    return (
      <div className="grid h-24 w-24 shrink-0 place-items-center gap-0.5 rounded-lg border border-dashed border-border bg-muted/40 text-center text-[9px] leading-tight text-muted-foreground">
        <ImageOff className="h-4 w-4" /> gambar<br />gak ke-load
      </div>
    );
  }
...
function DraftCard({ draft, onDone }: { draft: BufferDraft; onDone: () => void }) {
  ...
  const approve = useMutation({
    ...
    onSuccess: (_d, args) => { toast.success(args.mode === "now" ? "Diposting sekarang ✅" : args.mode === "schedule" ? "Dijadwalin ✅" : "Masuk antrian Buffer ✅"); onDone(); },
    onError: (e: any) => toast.error("Gagal approve", { description: e?.message }),
  });
  const reject = useMutation({
    mutationFn: () => nexusApi.bufferReject(draft.id),
    onSuccess: () => { toast.success("Draft ditolak & dihapus"); onDone(); },
    onError: (e: any) => toast.error("Gagal reject", { description: e?.message }),
  });
  ...
  return (
    <div className="flex flex-col rounded-2xl border border-border bg-card p-4 shadow-soft">
      ...
      <p className="flex-1 whitespace-pre-wrap rounded-xl bg-muted/40 p-3 text-sm leading-relaxed">{draft.text || <span className="text-muted-foreground italic">(tanpa caption)</span>}</p>
      ...
      {scheduling ? (
        <div className="mt-3 flex flex-wrap items-center gap-2">
          ...
          <button disabled={!dueAt || busy} onClick={() => approve.mutate({ mode: "schedule", dueAt: new Date(dueAt).toISOString() })} className="inline-flex items-center gap-1 rounded-lg bg-primary px-3 py-1.5 text-xs font-bold text-primary-foreground disabled:opacity-40">
            {approve.isPending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <CalendarClock className="h-3.5 w-3.5" />} Jadwalin
          </button>
          <button onClick={() => setScheduling(false)} className="rounded-lg border border-border px-3 py-1.5 text-xs font-semibold hover:bg-accent">Batal</button>
        </div>
      ) : (
        <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
          <button disabled={busy} onClick={() => approve.mutate({ mode: "queue" })} title="Masuk antrian Buffer (auto-jadwal)" className="inline-flex items-center justify-center gap-1 rounded-lg border border-emerald-200 bg-emerald-50 px-2 py-1.5 text-xs font-bold text-emerald-700 transition hover:bg-emerald-100 disabled:opacity-40">
            {pendMode === "queue" ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Clock className="h-3.5 w-3.5" />} Antrian
          </button>
          <button disabled={busy} onClick={() => setScheduling(true)} title="Pilih tanggal & jam tayang" className="inline-flex items-center justify-center gap-1 rounded-lg border border-sky-200 bg-sky-50 px-2 py-1.5 text-xs font-bold text-sky-700 transition hover:bg-sky-100 disabled:opacity-40">
            <CalendarClock className="h-3.5 w-3.5" /> Jadwal
          </button>
          <button disabled={busy} onClick={() => approve.mutate({ mode: "now" })} title="Posting sekarang juga" className="inline-flex items-center justify-center gap-1 rounded-lg border border-primary bg-primary/10 px-2 py-1.5 text-xs font-bold text-primary transition hover:bg-primary/20 disabled:opacity-40">
            {pendMode === "now" ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Send className="h-3.5 w-3.5" />} Sekarang
          </button>
          <button disabled={busy} onClick={() => { if (confirm("Tolak & hapus draft ini dari Buffer?")) reject.mutate(); }} title="Tolak & hapus" className="inline-flex items-center justify-center gap-1 rounded-lg border border-border px-2 py-1.5 text-xs font-bold text-muted-foreground transition hover:border-rose-300 hover:bg-rose-50 hover:text-rose-600 disabled:opacity-40">
            {reject.isPending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Trash2 className="h-3.5 w-3.5" />} Tolak
          </button>
        </div>
      )}
    </div>
